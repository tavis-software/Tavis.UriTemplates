...
namespace UriTemplateTests
{
    public class ParameterMatchingTests
    {
        ...
        public void MatchUriToTemplate()
        {
            ...
            string sTemplate = "http://example.com/{p1}/{p2}";

            string x = UriTemplate.CreateMatchingRegex(sTemplate);
            bool match = Regex.IsMatch(uri.AbsoluteUri,x);
            Assert.True(match);
        }
